...
WordVector::WordVector(ulint n, uint w){
...
void WordVector::setWord(ulint i, ulint W){
...
ulint WordVector::wordAt(ulint i){
...
void WordVector::setBit(ulint i, uint b){//set bit in position i
...
void WordVector::freeMemory(){
	if(n>0)
		delete [] v_64bit_words;
}

ulint WordVector::size(){//returns size of the structure in bits

	return n64*64;

}

void WordVector::saveToFile(FILE *fp){
...
#define check_numBytes() if (numBytes == 0) { VERBOSE_CHANNEL << "Read 0 bytes when reading dB-hash file (WordVector error)" << endl << flush; exit(1); }

void WordVector::loadFromFile(FILE *fp){
