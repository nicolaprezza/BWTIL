...
namespace bv
{
    ...
    template<template<typename ...> class C>
    std::string to_binary(bitview<C> const&v, size_t sep = 8, char ssep = ' ') {
    ...
    size_t bitview<Container>::popcount(size_t begin, size_t end) const
    {
            ...
            len -= step, p += step)
        {
            ...
            result += bv::popcount(get(p, p + step));
        }
    ...
    template<template<typename ...> class Container>
    void bitview<Container>::insert(size_t begin, size_t end, value_type value)
    {
        if(is_empty_range(begin, end))
            return;
